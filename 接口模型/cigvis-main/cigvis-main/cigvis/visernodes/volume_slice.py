from typing import List, Dict, Tuple, Union
import time
import numpy as np
from cigvis import colormap, ExceptionWrapper
import matplotlib.pyplot as plt
from matplotlib.colors import to_rgba as matplotlib_to_rgba
from cigvis.utils import utils
from cigvis.utils.slice_provider import SliceProvider
from cigvis import is_line_first

try:
    import viser
except BaseException as E:
    message = "run `pip install \"cigvis[viser]\"` or run `pip install \"cigvis[all]\"` to enable viser"
    viser = ExceptionWrapper(E, message)


_COLORMAP_EDGE_COLORS = {
    # Use distinct, high-contrast accents instead of sampling the colormap. A
    # sampled colour can disappear on the slice wherever the same value occurs.
    'gray': '#f59e0b',
    'grey': '#f59e0b',
    'greys': '#f59e0b',
    'seismic': '#22d3ee',
    'jet': '#f472b6',
    'petrel': '#8b5cf6',
    'stratum': '#2563eb',
    'faults': '#ef4444',
    'bwp': '#10b981',
}


def _colormap_edge_color(cmap):
    """Return a stable, contrasting edge colour for a colormap."""
    name = cmap if isinstance(cmap, str) else getattr(cmap, 'name', '')
    key = str(name).strip().lower()
    if key.endswith('_r'):
        key = key[:-2]
    return _COLORMAP_EDGE_COLORS.get(key, '#ffffff')


class VolumeSlice:

    def __init__(self,
                 volume,
                 axis='z',
                 pos=0,
                 cmap='gray',
                 clim=None,
                 scale=-1,
                 nancolor=None,
                 provider: SliceProvider = None):
        self._server = None  # viser.ViserServer
        self._name = f'slice-{axis}-{pos}'
        self.volume = volume
        self.provider = provider or SliceProvider(
            volume,
            transpose_line_first=False,
            transpose_rgb=False,
        )
        self.axis = axis
        self.pos = pos
        self.cmap = cmap
        self._cmap_preset = cmap
        self.clim = clim if clim is not None else utils.auto_clim(self.provider.clim_source)
        self.background_visible = True

        self.init_scale = [1.5 / max(self.provider.shape)] * 3
        self.nancolor = nancolor

        # lines
        self._observers: PosObserver = None  # observer the intersection lines
        self._img = None # save the original image (without white lines)

        # HACK: to deal with rgb or rgba image
        self.vol_shape = self.provider.shape

        if isinstance(scale, (int, float)):
            if scale < 0:
                self.scale = [1] * 3
            else:
                self.scale = [scale] * 3
        else:
            self.scale = scale
        self.scale = [s * x for s, x in zip(self.scale, self.init_scale)]

        self.update_node(self.pos)
        self.masks = []
        self.fg_cmaps = []
        self.fg_clims = []
        self._fg_cmaps_preset = []

    @property
    def server(self):
        return self._server

    @server.setter
    def server(self, server):
        if not isinstance(server, viser.ViserServer):
            raise ValueError("server must be type: viser.ViserServer")
        self._server = server
        self.update_node(self.pos)

    @property
    def name(self):
        return self._name

    @name.setter
    def name(self, name):
        self._name = str(name)
        self.update_node(self.pos)

    @property
    def render_width(self):
        if self.axis != 'z':
            return self.vol_shape[2] * self.scale[2]
        else:
            return self.vol_shape[1] * self.scale[1]

    @property
    def render_height(self):
        if self.axis == 'x':
            return self.vol_shape[1] * self.scale[1]
        else:
            return self.vol_shape[0] * self.scale[0]

    @property
    def wxyz(self):
        if self.axis == 'x':
            return (0.7071, 0.0, -0.7071, 0.0)
        elif self.axis == 'y':
            return (-0.5, 0.5, 0.5, 0.5)
        else:
            return (0, 0.7071, 0.7071, 0)

    @property
    def position(self):
        ni, nx, nt = self.vol_shape
        ri = ni * self.scale[0]
        rx = nx * self.scale[1]
        rt = nt * self.scale[2]
        # A smoke-test preview can legitimately contain one Inline, Crossline,
        # or sample.  Place that sole plane at the center of its axis instead
        # of dividing its zero-based position by ``count - 1``.
        inline_position = ri / 2 if ni <= 1 else self.pos * ri / (ni - 1)
        crossline_position = rx / 2 if nx <= 1 else self.pos * rx / (nx - 1)
        sample_position = rt / 2 if nt <= 1 else self.pos * rt / (nt - 1)
        if self.axis == 'x':
            return (inline_position, rx / 2, rt / 2)
        elif self.axis == 'y':
            return (ri / 2, crossline_position, rt / 2)
        else:
            return (ri / 2, rx / 2, sample_position)

    def _to_np(self, img):
        if utils.is_torch_tensor(img):
            img = img.detach().cpu().numpy()
        return img

    def _auto_transpose(self, img):
        if not is_line_first():
            img = np.transpose(img, (1, 0, 2))
        return img

    def to_img(self):
        bg = self.provider(self.axis, self.pos)
        fg = [mask(self.axis, self.pos) for mask in self.masks]

        bg = self._to_np(bg)
        fg = [self._to_np(g) for g in fg]

        if self.background_visible or not fg:
            arrays = [bg] + fg
            cmaps = [self.cmap] + self.fg_cmaps
            clims = [self.clim] + self.fg_clims
        else:
            arrays = fg
            cmaps = self.fg_cmaps
            clims = self.fg_clims
        img = colormap.arrs_to_image(arrays, cmaps, clims, True,
                                     self.nancolor)
        return self._auto_transpose(img)

    def set_background_visible(self, visible: bool):
        """Show the seismic base or render only the attached mask volumes."""
        visible = bool(visible)
        if visible == self.background_visible:
            return False
        if not visible and not self.masks:
            raise ValueError("prediction-only mode requires at least one mask")
        self.background_visible = visible
        self.update_node(self.pos)
        return True

    def add_observer(self, observer):
        self._observers = observer

    def update_node(self, pos):
        if self.server is None:
            return
        # startt = time.perf_counter()
        self.pos = int(pos)
        self._check_bound()
        self._img = self.to_img()
        img = self._decorate_with_observer(update_others=True)
        # midt = time.perf_counter()
        self._update(img)

    def update_line(self):
        if self.server is None:
            return
        img = self._decorate_with_observer(update_others=False)
        self._update(img)

    def _decorate_with_observer(self, update_others=False):
        """Draw margins/intersections without modifying the cached slice image."""
        if self._observers is None:
            return self._img

        img = self._img.copy()
        color = self._observers.color
        width = self._observers.width
        if width > 0:
            img[:width, :, :] = color
            img[-width:, :, :] = color
            img[:, :width, :] = color
            img[:, -width:, :] = color
        self._observers.set_pos(self.axis, self.pos)
        return self._observers.update_line(
            self.axis,
            img,
            update_others,
        )

    def _update(self, img):
        self.nodes = self.server.scene.add_image(
            self.name,
            img,
            self.render_width,
            self.render_height,
            format='png',
            wxyz=self.wxyz,
            position=self.position,
        )
        # endt = time.perf_counter()
        # print(
        #     f"update_node:{(midt - startt)*1000:.3f}ms, {(endt - midt)*1000:.3f}ms"
        # )

    @property
    def limit(self):
        if self.axis == 'x':
            return (0, self.vol_shape[0])
        elif self.axis == 'y':
            return (0, self.vol_shape[1])
        else:
            return (0, self.vol_shape[2])

    def _check_bound(self):
        if self.pos < 0:
            self.pos = 0
        if self.pos >= self.limit[1]:
            self.pos = self.limit[1] - 1

    def update_cmap(self, cmap):
        if cmap is None:
            cmap = self._cmap_preset
        self.cmap = cmap
        if self._observers is not None:
            self._observers.set_cmap(cmap)
        self.update_node(self.pos)

    def update_clim(self, clim):
        self.clim = clim
        self.update_node(self.pos)

    def update_mask_clim(self, clim, num):
        if len(self.masks) == 0 or num >= len(self.masks):
            return 
        self.fg_clims[num] = clim 
        self.update_node(self.pos)

    def update_mask_cmap(self, cmap, num):
        if len(self.masks) == 0 or num >= len(self.masks):
            return 
        self.fg_cmaps[num] = cmap 
        self.update_node(self.pos)

    def update_scale(self, scale):
        if isinstance(scale, (int, float)):
            scale = [scale] * 3
        self.scale = [s * x for s, x in zip(scale, self.init_scale)]

        self.update_node(self.pos)

    def add_mask(self,
                 vol,
                 cmap: str,
                 clim: List = None,
                 provider: SliceProvider = None):
        provider = provider or SliceProvider(
            vol,
            transpose_line_first=False,
            transpose_rgb=False,
        )
        assert provider.shape == self.vol_shape, f"mask.shape: {provider.shape} != vol.shape: {self.vol_shape}"
        self.masks.append(provider)
        if clim is None:
            clim = utils.auto_clim(provider.clim_source)
        self.fg_cmaps.append(cmap)
        self._fg_cmaps_preset.append(cmap)
        self.fg_clims.append(clim)
        self.update_node(self.pos)



class PosObserver:
    def __init__(self, color=(1, 1, 1), width=1) -> None:
        self.color = self.to_rgba(color)
        self.width = int(width)
        self.xaxis = 0
        self.yaxis = 0
        self.zaxis = 0
        self._shape = None
        self._linked_images = {}

    def set_pos(self, axis, pos):
        if axis == 'x':
            self.xaxis = pos 
        elif axis == 'y':
            self.yaxis = pos 
        else:
            self.zaxis = pos

    def link_image(self, image: VolumeSlice):
        self._linked_images[image.axis] = image
        self.set_pos(image.axis, image.pos)
        image.add_observer(self)
        self._shape = image.vol_shape

    def set_color(self, color, refresh=True):
        """Set the shared line colour and refresh linked slices once each."""
        rgba = self.to_rgba(color)
        if rgba == self.color:
            return False
        self.color = rgba
        if refresh:
            self.refresh()
        return True

    def set_cmap(self, cmap, refresh=True):
        """Select the edge accent associated with ``cmap``."""
        return self.set_color(_colormap_edge_color(cmap), refresh=refresh)

    def refresh(self):
        """Redraw linked slices without propagating observer updates again."""
        for image in tuple(self._linked_images.values()):
            image.update_line()

    def update_line(self, axis, img_orig: np.ndarray, update_others = False):
        img = img_orig.copy()
        half_width = self.width // 2

        def draw_line(arr, index, is_row):
            start = max(0, index - half_width)
            end = min(arr.shape[0] if is_row else arr.shape[1], index + half_width + 1)
            
            if not is_row:
                arr[start:end, :] = self.color
            else:
                arr[:, start:end] = self.color

        if axis == 'x':
            draw_line(img, self.yaxis, False)
            draw_line(img, self.zaxis, True)
            if update_others:
                if 'y' in self._linked_images:
                    self._linked_images['y'].update_line()
                if 'z' in self._linked_images:
                    self._linked_images['z'].update_line()
        elif axis == 'y':
            draw_line(img, self.xaxis, False)
            draw_line(img, self.zaxis, True)
            if update_others:
                if 'x' in self._linked_images:
                    self._linked_images['x'].update_line()
                if 'z' in self._linked_images:
                    self._linked_images['z'].update_line()
        else:
            draw_line(img, self.xaxis, False)
            draw_line(img, self.yaxis, True)
            if update_others:
                if 'x' in self._linked_images:
                    self._linked_images['x'].update_line()
                if 'y' in self._linked_images:
                    self._linked_images['y'].update_line()

        return img

    def to_rgba(self, color):
        try:
            if isinstance(color, str) or not any([c > 1 for c in color]):
                rgba = matplotlib_to_rgba(color)
                r, g, b, a = rgba
                r = int(round(r * 255))
                g = int(round(g * 255))
                b = int(round(b * 255))
                a = int(round(a * 255))
                return [r, g, b, a]
            else:
                if len(color) == 3:
                    color = list(color) + [1]
                return color
        except (ValueError, TypeError) as e:
            raise ValueError(f"Unknow color: {color}, {e}") from e
